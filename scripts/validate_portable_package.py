...
def validate_package(root: Path, forbidden_text: Iterable[str] = ()) -> list[str]:
    ...
    for path in root.rglob("*"):
        ...
        if path.is_dir():
            ...
            elif any(part in FORBIDDEN_OPTIONAL_RUNTIME_DIRECTORY_NAMES for part in parts) or any(
        ...
        elif name.endswith(".dll") and name.startswith(FORBIDDEN_FFMPEG_LIBRARY_PREFIXES):
...
def main(argv: list[str] | None = None) -> int:
    ...
    violations = validate_package(args.package_root, args.forbidden_text)
    if violations:
